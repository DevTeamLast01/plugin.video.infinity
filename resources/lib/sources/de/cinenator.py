...
class source:
    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            result = dom_parser.parse_dom(result, 'div', attrs={'id': 'views'})
            result = dom_parser.parse_dom(result, 'tbody')
            r = re.findall(r'domain=(.*?)".*?"(.*?)".*?\<td\>(.*?)\<', result[0].content, flags=re.DOTALL)


            for hoster, link, quality in r:
                result = self.scraper.get(link).content
                url = re.findall(r"window.location.href='(.*?)'", result)[0]

                valid, hoster = source_utils.is_host_valid(url, hostDict)
                if valid:
                    sources.append({'source': hoster, 'quality': quality, 'language': 'de', 'Info': '', 'url': url, 'direct': False,
