    ...
    def sources(self, url, hostDict, hostprDict):
        ...
        try:
            ...
            if rels and len(rels) > 1:
                r = []
                for rel in rels:
                    oRequest = cRequestHandler(urlparse.urljoin(self.base_link, rel.attrs['href']))
                    oRequest.removeBreakLines(False)
                    oRequest.removeNewLines(False)
                    relData = oRequest.request()
                    ...
                    for i in relData:
                        oRequest = cRequestHandler(quurlparse.urljoin(self.base_link, i.attrs['href'])ery)
                        oRequest.removeBreakLines(False)
                        oRequest.removeNewLines(False)
                        i = oRequest.request()
